...
当前自动验证为 130 项 unittest，另有引用/边界检查、Python compileall 和
`git diff --check`。
...
2026-07-31 按用户要求暂不修改 R9。固定回归 run
...
该问题属于 R9 本地关联、恢复和运动确认边界，不属于 `src/court`。后续只有在
用户明确要求修改 R9 时才能处理；处理前应先把 808～825 帧构造成固定检测序列
回归，优先限制“长失联、未成熟轨迹、低置信度、大位移恢复”，避免用全局阈值
破坏真实高速球和正常向下预测。
